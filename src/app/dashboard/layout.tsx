    ...
    const getTeams = async () => {
      try {
        ...
        if (mappedTeams.length > 0 && !currentTeam) {
          ...
          const firstVotableTeam =
            mappedTeams.find((team) => team.canVote) || mappedTeams[0];
    ...
    >
      ...
      {!isDesktop && !sidebarOpen && (
        <button
          ...
          className="fixed top-6 right-6 z-50 p-3 rounded-full bg-primary/40 backdrop-blur-md text-primary-foreground shadow-md hover:shadow-lg transition-all"
        >
          <svg
